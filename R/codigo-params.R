...
#'   Default is `v2`.
#' @param release A string specifying the release version to search from. If not
...
#'   Currently, the possible values are *"mms"* and *"icf"*. Default is *"mms"*.
#' @param language ICD-API is multi-lingual. Language codes such as "en", "es",
#'   "zh", etc. Default is English ("en").
#' @param tabular Logical. Should output be structured into a tibble? Default
#'   is TRUE.
#' @param verbose Logical. Should non-warning and non-error messages be printed?
#'   Default is TRUE.
...
#' @param max_tries Maximum number of times to retry a request if it fails with
#'   a transient error (e.g. 429, 500, 503). Default is 3.
#'
#' @name codigo-params
#' @keywords internal
#'
